...
export function SingleEmployeeForm({ formData, onFormChange, selectedTemplate }: SingleEmployeeFormProps) {
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      try {
        // Step 1: Remove background (single mode: PhotoRoom → WASM → original)
        console.log('🎨 Step 1: Removing background...');
        const result = await removeImageBackground(file, null, 'single_' + Date.now());
        const processedFile = result.file;
        const bgRemoved = result.method === 'photoroom' || result.method === 'wasm';
