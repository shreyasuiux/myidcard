...
export function SettingsModal({ isOpen, onClose }: SettingsModalProps) {
  ...
  useEffect(() => {
    ...
    const savedKey = localStorage.getItem('photoroom_api_key') || '';
    setRemoveBgApiKey(savedKey);
  }, [isOpen]);
  
  const handleSaveApiKey = () => {
    localStorage.setItem('photoroom_api_key', removeBgApiKey.trim());
  };
                    ...
                    <div className="space-y-4 pt-6 border-t border-slate-700/50">
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <p className="text-sm text-yellow-300 font-medium mb-2">
                              <strong>Upgrade to Professional Quality!</strong>
                            </p>
                            <p className="text-xs text-slate-300 leading-relaxed">
                              For <strong>superior portrait background removal</strong> optimized for HR ID cards, add your <strong>PhotoRoom API key</strong>.
                              Without it, the system uses local AI processing (good accuracy). PhotoRoom is #1 rated for professional headshots with 100 free images/month. Get your API key at{' '}
                              <a 
                      ...
                      <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
                        ...
                        <p className="text-xs text-slate-500 mt-2">
                          API key is stored locally and never shared. 100 free images/month. Saved automatically on blur.
                        </p>
                      </div>
                    </div>
                  </div>
