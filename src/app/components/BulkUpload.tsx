  ...
  const processZipFile = async (file: File, employees: EmployeeRecord[]) => {
    ...
    try {
      ...
      if (result.isValid && parseResult) {
        ...
        // Check for warnings (e.g., bg removal skipped)
        ...
        // Success toast - mention warnings if any
        if (warningCount > 0) {
          toast.success('All photos matched and ready for export!', {
            description: `${result.matches.length} photos linked. ${warningCount} photo(s) processed without background removal.`,
          });
        } else {
