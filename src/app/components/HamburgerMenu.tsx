...
import { X, User, LogOut, Database, FileText, Settings, HelpCircle, Sparkles, Layers } from 'lucide-react';
          ...
          >
            ...
            <div className="p-4 space-y-2">
              ...
              <motion.button
                className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-slate-800/30 hover:bg-slate-800/60 text-left transition-colors touch-manipulation"
                whileTap={{ scale: 0.98 }}
                onClick={() => { onClose(); navigate('/engine-playground'); }}
              >
                <div className="w-10 h-10 rounded-lg bg-violet-500/10 flex items-center justify-center">
                  <Layers className="w-5 h-5 text-violet-400" />
                </div>
                <div>
                  <p className="text-sm font-medium text-white">Engine Playground</p>
                  <p className="text-xs text-slate-400">Template rendering lab</p>
                </div>
              </motion.button>
            </div>
