...
import { getPhotoOverlayParams, verifyPhotoSource } from './photoOverlay';
  ...
  try {
    ...
    if (includeFront && frontElement) {
      ...
      if (employee.photoBase64 && employee.photoBase64.startsWith('data:image/')) {
        ...
        // TEMPLATE-DRIVEN photo position — reads from active template, NEVER hardcoded
        if (!template) {
          throw new Error('Template is required for photo overlay positioning');
        }
        const photoOverlay = getPhotoOverlayParams(template, CARD_WIDTH_MM, CARD_HEIGHT_MM);
        verifyPhotoSource(employee.name, employee.photoBase64, template.name);

        if (!photoOverlay.visible) {
          console.log('   ⚠️ Photo hidden by template — skipping overlay');
        } else {
          const photoWidthMM = photoOverlay.widthMM;
          const photoHeightMM = photoOverlay.heightMM;
          ...
          console.log(`   Photo position: (${photoOverlay.leftPx}, ${photoOverlay.topPx})px → (${photoOverlay.leftMM.toFixed(2)}, ${photoOverlay.topMM.toFixed(2)})mm`);
          ...
          // Embed ORIGINAL photo at TEMPLATE-DRIVEN position
          pdf.addImage(
            employee.photoBase64,    // ORIGINAL base64 (bg-removed + cropped)
            'PNG',                   // Format (PNG for transparency)
            photoOverlay.leftMM,     // X from template
            photoOverlay.topMM,      // Y from template
            photoOverlay.widthMM,    // Width from template
            photoOverlay.heightMM,   // Height from template
            undefined,               // Alias
            PDF_COMPRESSION          // NO compression
          );
          
          console.log('   ✅ ORIGINAL photo embedded at TEMPLATE-DRIVEN position');
